...
parser = argparse.ArgumentParser(description='submit the RAW step for 2023 SUEP MC')
parser.add_argument(
    '--fileList',
    '-f',
    required=True,
    type=str,
    nargs='?',
    help='List of files to perform the raw step on'
)
parser.add_argument(
    '--nFilesPerJob',
    '-n',
...
jobName = f'SUEP2023_RAW_{todaysDate}'
...
config = f'{os.environ["CMSSW_BASE"]}/src/MCProduction/SUEPProduction/python/SUEP_DIGIPREMIX_2023.py'
